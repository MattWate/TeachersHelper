      ...
      <section className="workspace">
        <aside className="panel learner-panel">
          ...
          <div className="learner-list">
            {classLearners.length === 0 ? <p className="empty-state">Add learners to start capturing observations.</p> : classLearners.map((learner) => {
              const count = dashboard.observations.filter((observation) => observation.learner_id === learner.id).length;
              return <button key={learner.id} className={learner.id === selectedLearnerIdResolved ? 'learner-item active' : 'learner-item'} onClick={() => { setSelectedLearnerId(learner.id); setReport(null); }}><span>{learner.full_name}</span><small>{count} notes</small></button>;
            })}
          </div>
        </aside>
        ...
        <section className="panel capture-panel">
          ...
          <form onSubmit={addObservation} className="note-form">
            <div className="segmented-control" aria-label="Observation type">
              ...
              <button type="button" className={noteType === 'voice' ? 'active' : ''} onClick={() => setNoteType('voice')}>Voice note</button>
            </div>
          ...
          <div className="observation-list">
            ...
            {learnerObservations.length === 0 ? <p className="empty-state">No observations yet. Add one to start building this learner's classroom memory.</p> : learnerObservations.map((observation) => <article key={observation.id} className="observation-card"><div><strong>{observation.category || 'General'}</strong><span>{observation.sentiment || 'Observation'}</span></div><p>{observation.cleaned_text || observation.original_text}</p><small>{new Date(observation.created_at).toLocaleString()}</small></article>)}
          </div>
        </section>
